...
function guardReadOnlyProjection(
  ...
  const readOperatorNoteContent = readOnly.readOperatorNoteContent;
  return Object.freeze({
    lastSynced: async () => {
      ensureOpen();
    ...
    ...(hasNoteInNotebook === undefined
      ...
      : {
          hasNoteInNotebook: async (notebookId: string, noteId: string) => {
            ensureOpen();
            return hasNoteInNotebook(notebookId, noteId);
          },
        }),
    ...(readNoteLockState === undefined
      ? {}
      : {
          readNoteLockState: async (id: string) => {
            ensureOpen();
            return readNoteLockState(id);
          },
        }),
    ...(noteContentDiagnostic === undefined
      ? {}
      : {
    ...
    ...(readOperatorNoteContent === undefined
      ? {}
      : {
          readOperatorNoteContent: async (id: string) => {
            ensureOpen();
            return readOperatorNoteContent(id);
          },
        }),
