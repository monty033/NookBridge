...
function makeFakeArtifact(root: string): {
  ...
  const artifact = join(root, "nookbridge-v0.1.3-linux-x64-gnu.tar.gz");
  ...
  execFileSync("mkdir", [
    "-p",
    join(artifactDir, "nookbridge-v0.1.3", "bin"),
    join(artifactDir, "nookbridge-v0.1.3", "scripts"),
  ]);
  writeFileSync(
    join(artifactDir, "nookbridge-v0.1.3", "release.json"),
    `${JSON.stringify({ version: "0.1.3", target: "linux-x64-gnu" }, null, 2)}\n`,
  );
  writeFileSync(
    join(artifactDir, "nookbridge-v0.1.3", "SHA256SUMS"),
  ...
  for (const name of ["nookctl", "nookbridge-provision", "nookbridge-sync", "nookbridge-health"]) {
    writeFileSync(
      join(artifactDir, "nookbridge-v0.1.3", "bin", name),
  ...
  writeFileSync(
    join(artifactDir, "nookbridge-v0.1.3", "scripts", "install-systemd.sh"),
    fakeInstallerScript,
    { mode: 0o755 },
  );
  execFileSync("tar", ["-czf", artifact, "-C", artifactDir, "nookbridge-v0.1.3"]);
  ...
  writeFileSync(checksumFile, `${sha256(artifact)}  nookbridge-v0.1.3-linux-x64-gnu.tar.gz\n`);
...
function buildFakeInstallerEnv(): {
  ...
  writeFileSync(
    checksumFile,
    [
      `${sha256(artifact)}  nookbridge-v0.1.3-linux-x64-gnu.tar.gz`,
...
describe("GitHub one-command installer bootstrap", () => {
  ...
  it("binds options before subcommand and validates the artifact", () => {
    ...
    expect(log).toMatch(/--artifact \/[^ ]*nookbridge-v0\.1\.3-linux-x64-gnu\.tar\.gz/);
